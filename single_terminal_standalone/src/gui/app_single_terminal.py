...
# 定数
COLLECT_SAMPLES = 90          # 1回の収集で取るサンプル数
CALIBRATION_SAMPLES = 10      # キャリブレーション時のサンプル数
...
class SingleTerminalApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        if self.classifier.load():
            ...
            # スイープモードをモデルに合わせて同期
            self._sweep_mode = self.classifier.use_sweep
            self.sweep_var.set(self.classifier.use_sweep)
            if self._sweep_mode:
                self.sweep_info_label.configure(
                    text="ON: Xピーク周波数 + mag + phase (10D)",
                    text_color="#ffaa00")
            else:
                self.sweep_info_label.configure(
    ...
    def _start_monitoring(self):
        ...
        # キャリブレーション開始 (ベースラインドリフト補正)
        self.classifier.reset_calibration()
        self._calibration_buf: list = []
        self._calibration_remaining = CALIBRATION_SAMPLES
        self._set_status("Calibrating... (keep RELEASED)", "yellow")
        self.press_label.configure(text="CALIBRATING", text_color="yellow")
        self._calibrate_step()

    def _calibrate_step(self):
        """キャリブレーション中のサンプル収集"""
        if not self.monitoring or self._calibration_remaining <= 0:
            self._finish_calibration()
            return
        try:
            sweep_features, mag, phase = self._measure_once()
            feat = self.classifier._to_features(mag, phase, sweep_features)
            self._calibration_buf.append(feat)
            self._calibration_remaining -= 1
            self.after(50, self._calibrate_step)
        except Exception as e:
            logger.error(f"キャリブレーションエラー: {e}")
            self._finish_calibration()

    def _finish_calibration(self):
        """キャリブレーション完了 → スケーラー補正 → 監視開始"""
        if self._calibration_buf:
            try:
                result = self.classifier.calibrate(self._calibration_buf)
                drift_norm = result['drift_norm']
                logger.info(f"ドリフトノルム: {drift_norm:.2f}σ")
                if drift_norm > 3.0:
                    self._set_status(
                        f"Calibrated (drift={drift_norm:.1f}σ - large!)", "orange")
                else:
                    self._set_status(
                        f"Monitoring (drift={drift_norm:.1f}σ)", "#00ff00")
            except Exception as e:
                logger.error(f"キャリブレーション失敗: {e}")
                self._set_status("Monitoring (no calibration)", "#00ff00")
        else:
