...
def test_classifier():
    ...
    try:
        ...
        logger.info(f"モデル情報: {classifier.get_model_info()}")
        
        # データソース接続
        data_source = DataSourceFactory.create()
        if not data_source.connect():
            logger.error("データソース接続失敗")
            return
        
        # グリッド位置を取得
        grid_positions = classifier.get_grid_positions()
        ...
        for class_id, (x, y) in enumerate(grid_positions):
            ...
            # 確率予測
            probabilities = classifier.predict_probabilities(impedance_vector)
            predicted_class = classifier.predict_class(impedance_vector)
            
            logger.info(f"\nクラス {class_id}: ({x:.2f}, {y:.2f}) mm")
            logger.info(f"  予測クラス: {predicted_class}")
            logger.info(f"  確率分布: {probabilities}")
            logger.info(f"  最大確率: {probabilities[class_id] * 100:.2f}%")
