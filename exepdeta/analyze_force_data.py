...
def create_combined_plot(force_displacement_data, peak_forces_data, concentration, output_path):
    ...
    # 濃度内の全データをまとめてプロット
    ax1.plot(force_displacement_data['displacement_mm'], force_displacement_data['force_N'], 
            color=line_color, alpha=0.6, linewidth=1.0, label=f'{concentration}')
    ...
    # 箱ひげ図をプロット（1つのボックス）
    bp = ax2.boxplot([all_peaks], labels=[concentration], patch_artist=True, widths=0.6)
    ...
    ax2.set_title(f'{concentration} - ピーク力の分布', fontsize=14, fontweight='bold')
...
def create_comparison_plot(concentration_files, output_dir):
    ...
    # 箱の色を設定
    colors = plt.cm.Set2(np.linspace(0, 1, len(concentrations)))
